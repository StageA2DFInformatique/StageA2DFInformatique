...
/**
 * Description of Semaine3DAO
 * Classe métier : Semaine3
 * @author btssio
 */
class Semaine3DAO implements IDAO {
    ...
    protected static function enregVersMetier($enreg) {
        ...
        $uneVente1 = new Semaine3($id, $designation, $type, $prix);
    ...
    public static function insert($objet) {
        $requete = "INSERT INTO Semaine3 VALUES (:id, :designation, :type, :prix)";
    ...
    public static function update($id, $objet) {
        $ok = false;
        $requete = "UPDATE  Semaine3 SET DESIGNATION=:designation, TYPE=:type, PRIX=:prix WHERE ID=:id";
        $stmt = Bdd::getPdo()->prepare($requete);
        self::metierVersEnreg($objet, $stmt);
    ...
    public static function delete($id) {
        $ok = false;
        $requete = "DELETE FROM Semaine3 WHERE ID = :id";
        $stmt = Bdd::getPdo()->prepare($requete);
        $stmt->bindParam(':id', $id);
        $ok = $stmt->execute();
    ...
    public static function getOneById($id) {
        $objetConstruit = null;
        $requete = "SELECT * FROM Semaine3 WHERE ID = :id";
        $stmt = Bdd::getPdo()->prepare($requete);
        $stmt->bindParam(':id', $id);
        $ok = $stmt->execute();
    ...
    public static function isAnExistingId($id) {
        $requete = "SELECT COUNT(*) FROM Semaine3 WHERE ID=:id";
        $stmt = Bdd::getPdo()->prepare($requete);
        $stmt->bindParam(':id', $id);
        $stmt->execute();
