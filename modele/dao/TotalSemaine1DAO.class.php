 ...
 * @author btssio
 */
class TotalSemaine1DAO implements IDAO {
     ...
     * @param TotalSemaine1 $objet objet métier à insérer
     * @return boolean =FALSE si l'opérationn échoue
     */
    public static function insert($objet) {
        $requete = "INSERT INTO TotalSemaine1 VALUES (:id, :total)";
     ...
     * @param TotalSemaine1 $objet objet métier à mettre à jour
     * @return boolean =FALSE si l'opération échoue
     */
    public static function update($id, $objet) {
        $ok = false;
        $requete = "UPDATE  TotalSemaine1 SET TOTAL=:total, 
                WHERE ID=:id";
        $stmt = Bdd::getPdo()->prepare($requete);
        self::metierVersEnreg($objet, $stmt);
    ...
    public static function delete($id) {
        $ok = false;
        $requete = "DELETE FROM TotalSemaine1 WHERE ID = :id";
        $stmt = Bdd::getPdo()->prepare($requete);
        $stmt->bindParam(':id', $id);
        $ok = $stmt->execute();
    ...
    public static function getAll() {
        $lesObjets = array();
        $requete = "SELECT * FROM TotalSemaine1";
        $stmt = Bdd::getPdo()->prepare($requete);
        $ok = $stmt->execute();
        if ($ok) {
    ...
    public static function getOneById($id) {
        $objetConstruit = null;
        $requete = "SELECT * FROM TotalSemaine1 WHERE ID = :id";
        $stmt = Bdd::getPdo()->prepare($requete);
        $stmt->bindParam(':id', $id);
        $ok = $stmt->execute();
    ...
    public static function isAnExistingId($id) {
        $requete = "SELECT COUNT(*) FROM TotalSemaine1 WHERE ID=:id";
        $stmt = Bdd::getPdo()->prepare($requete);
        $stmt->bindParam(':id', $id);
        $stmt->execute();
